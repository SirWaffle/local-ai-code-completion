...
using genLib.Tokenizers;

namespace genLib.Generators
{
    public class CodeGenOnnx
    {
        ...
        public void Load(int gpuDevice, string onnxModelPath, ITokenizer _tokenizer)
        {
        ...
        //TODO: beam search
        //TODO: add batches
        public List<string> DoInference(CodeGenerationSettings gen)
        {
            //tokenize
            long[] promptTokenIds = tokenizer!.Tokenize(gen.prompt);
            ...
            if (gen.stopping_criteria != null && gen.stopping_criteria.Length > 0)
            {
                ...
                {
                    stoppingTokens.Add(tokenizer!.Tokenize(str));
                }
            }
            ...
            for (int i = 0; i < gen.max_length; ++i)
            {
                ...
                //inputs for session                
                input[0] = NamedOnnxValue.CreateFromTensor("input_ids", inputTensor);
                input[1] = NamedOnnxValue.CreateFromTensor("attention_mask", atnTensor);
            ...
            string text = tokenizer!.TokenIdsToText(toks);
